...
# 强制任务：仅输出符合格式的JSON食谱推荐（无任何多余内容）
用户查询（query）：{{query}}
## 必须遵守的规则：
1. 从RAG结果选≤3个推荐（distance≤0.4），理由需包含1个具体食材/步骤，自然改写（禁止复制原文）；
2. 每个推荐必填：doc_id、Name、Calories、Total Time、Reason、taste（口味从指定列表选：sour/sweet/bitter/spicy/salty/略XX）；
3. 最终仅输出JSON数组，**不允许重复输出任何内容、不添加任何注释/模板文字、不包裹```json标签**；
4. 若违反以上规则，本次回答无效。
